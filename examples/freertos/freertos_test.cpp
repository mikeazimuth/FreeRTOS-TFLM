...
namespace {
using TflmOpResolver = tflite::MicroMutableOpResolver<1>;

TfLiteStatus RegisterOps(TflmOpResolver& op_resolver) {
...
TfLiteStatus ProfileMemoryAndLatency() {
  ...
  tflite::RecordingMicroInterpreter interpreter(
      tflite::GetModel(g_float_model_data), op_resolver, allocator,
...
TfLiteStatus LoadFloatModelAndPerformInference() {
  const tflite::Model* model =
      ::tflite::GetModel(g_float_model_data);
...
TfLiteStatus LoadQuantModelAndPerformInference() {
  ...
  const tflite::Model* model =
      ::tflite::GetModel(g_int8_model_data);
  TFLITE_CHECK_EQ(model->version(), TFLITE_SCHEMA_VERSION);
